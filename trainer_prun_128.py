...
def run_model_one_epoch(model, data_loader, criterion, device, train=False, optimizer=None):
    ...
    for step, sampled_batch in tqdm(enumerate(data_loader), total=len(data_loader)):
        ...
        loss = criterion(outputs, labels, softmax=True)
        if train:
            loss.backward()
            optimizer.step()
...
def trainer_synapse(args, model, snapshot_path):
    ...
    for epoch_num in iterator:
        ...
        pruning_recipe_path = './prun.yaml' 
        manager = ScheduledModifierManager.from_yaml(pruning_recipe_path)
        optimizer = manager.modify(model, optimizer, steps_per_epoch=len(trainloader))  
        ...
        for epoch in range(manager.max_epochs):
          ...
          train_loss = run_model_one_epoch(model, trainloader, dice_loss, device, train=True, optimizer=optimizer)
          print(f"Training Epoch: {epoch_name}\nTraining Loss: {train_loss}\n")

          

        print(f'#'*100)
        print(f'PRUNNING FINALIZED')
        print(f'#'*100)
        ...
        exporter.export_pytorch(name="Embedded_ViT_128-pruned.pth")
        exporter.export_onnx(torch.randn(1, 1, 128, 128), name="Embedded_ViT_128-pruned.onnx", convert_qat=True)
        
        # Test
        eval_interval = args.eval_interval 
        if epoch_num >= int(max_epoch / 2) and (epoch_num + 1) % eval_interval == 0:
            filename = f'{args.model_name}_PRUN_epoch_{epoch_num}.pth'
            save_mode_path = os.path.join(snapshot_path, filename)
            model.eval()
