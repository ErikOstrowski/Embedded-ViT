...
from networks.segformer_nogroup import *
...
class MyDecoderLayern(nn.Module):
    def __init__(
        self, input_size, in_out_chan, head_count, token_mlp_mode, n_class=5, norm_layer=nn.LayerNorm, is_last=False
    ):
        super().__init__()
        dims = in_out_chan[0]
        out_dim = in_out_chan[1]
...
class EmbeddedViT(nn.Module):
    def __init__(self, num_classes=9, head_count=1, token_mlp_mode="mix_skip"):
        ...
        # Encoder
        dims, key_dim, value_dim, layers = [[32 , 80, 128], [32 , 80, 128], [32 , 80, 128], [1, 1, 1]]
        self.backbone = MiT(
            image_size=64,
            in_dim=dims,
            key_dim=key_dim,
        ...
        self.decoder_2 = MyDecoderLayer(
            (d_base_feat_size * 1, d_base_feat_size * 1),
            in_out_chan[2],
            head_count,
            token_mlp_mode,
            n_class=num_classes,
        )
        self.decoder_1 = MyDecoderLayern(
    ...
    def forward(self, x):
        ...
        b, c, _, _ = output_enc[1].shape

        # ---------------Decoder-------------------------
        tmp_2 = self.decoder_2(output_enc[1].permute(0, 2, 3, 1).view(b, -1, c))
        tmp_1 = self.decoder_1(tmp_2, output_enc[0].permute(0, 2, 3, 1))

        tmp_1 = self.dequant(tmp_1)
        return tmp_1
